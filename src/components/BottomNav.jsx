export default function BottomNav({ activeTab, onTabChange, isTrainer, pendingCount = 0 }) {
  ...
  return (
    <nav
      dir="rtl"
      className="bg-white border-t border-gray-200 shadow-lg flex"
      style={{
        position: 'fixed',
        bottom: 0,
        left: 0,
        ...
        return (
          ...
          >
            ...
            {tab.id === 'shop' && isTrainer && pendingCount > 0 && (
              <span
                className="absolute top-1.5 flex items-center justify-center rounded-full bg-red-500 text-white text-[10px] font-bold"
                style={{ right: '22%', width: 16, height: 16 }}
              >
                {pendingCount}
              </span>
            )}
